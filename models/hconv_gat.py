...
class HConv_GAT(nn.Module):
    def __init__(self, vocab_size, max_seq_len, drop_rate, hconv_gnn, gat_dims, num_heads,
                 embedding_dim, window_sizes, dilations, pre_cnn_dims, pre_gnn_dims,
                 pred_dims, readout_pool:str='sum', init_weight=None, need_embed:bool=False,
                 activation=None, pred_act:str='ELU', mode='concat', freeze:bool=False, residual=False, **kwargs):
        super(HConv_GAT, self).__init__()
        assert len(dilations) == len(window_sizes) == len(pre_cnn_dims) == len(pre_gnn_dims)
        assert len(gat_dims) == len(num_heads)
        ...
        for pre_cnn_dim, pre_gnn_dim, dilation, window_size in zip(pre_cnn_dims, pre_gnn_dims, dilations, window_sizes):
            self.pre_hconv_layers.append(
                HConvLayer(in_dim, pre_cnn_dim, pre_gnn_dim, window_size, dilation, hconv_gnn,
                           activation=self.activation, residual=residual, **kwargs)
            )
        ...
        out_dim = 0
        for gat_dim, num_head in zip(gat_dims, num_heads):
            self.gat_layers.append(
                GATLayer(in_dim, gat_dim, num_head, activation=activation, residual=residual, last_layer=False)
            )
            in_dim = gat_dim
            out_dim += in_dim
        out_dim += in_dim * (len(gat_dims) - 1)
