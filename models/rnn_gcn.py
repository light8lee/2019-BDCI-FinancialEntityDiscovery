...
class RNN_GCN(nn.Module):
    def __init__(self, vocab_size, max_seq_len, drop_rate,
                 embedding_dim, hidden_dims:list, pred_dims:list=None, num_rnn_layer:int=1,
...
class GRU_GCN(RNN_GCN):
    def __init__(self, vocab_size, max_seq_len, drop_rate,
                 embedding_dim, hidden_dims, pred_dims, num_rnn_layer=1,
                 init_weight=None, activation=None,
                 pred_act='ELU', residual=False, freeze=False, **kwargs):
        super(GRU_GCN, self).__init__(vocab_size, max_seq_len, drop_rate,
                                      embedding_dim, hidden_dims, pred_dims, num_rnn_layer,
                                      init_weight, activation, pred_act,
                                      residual, freeze, **kwargs)
