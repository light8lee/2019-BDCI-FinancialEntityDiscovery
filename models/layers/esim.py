...
class ESimLayer(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.project = nn.Linear(out_dim*4, out_dim)
        self.rnn2 = nn.LSTM(out_dim, out_dim//2, 1, bidirectional=True, batch_first=True)
    ...
    def forward(self, inputs_a, inputs_b, masks_a, masks_b):
        ...
        attn_a = torch.softmax(attn-1e9*(1-masks_b).transpose(-1, -2), -1)  # [b, t1, t2]
        attn_a = torch.matmul(attn_a, outputs_b)  # [b, t1, h]
        attn_b = torch.softmax(attn.transpose(-1, -2)-1e-9*(1-masks_a).transpose(-1, -2), -1)  # [b, t2, t1]
